...
#
# __gnu_cxx::hash_map<>  by xxf
#
define phmap
 if $argc == 0
  printf "phamp <hash_map var name>\n"
 else
  printf "===================hash_map begin==========\n"
  set $alloc = $arg0._M_ht._M_node_allocator
  set $hasher = $arg0._M_ht._M_hash
  set $equals = $arg0._M_ht._M_equals
  set $elem_num = $arg0._M_ht._M_num_elements
  set $vecsize = $arg0._M_ht._M_buckets._M_impl._M_finish - $arg0._M_ht._M_buckets._M_impl._M_start
  
  printf "hash_map allocator:"
  p $alloc
  printf "hash_map hasher:"
  p $hasher
  printf "hash_map equals:"
  p $equals
  printf "hash_map element num:%u\n", $elem_num
  printf "bucket size:%u\n",$vecsize
  
  set $i = 0
  set $elemidx = 0
  printf "elem in bucket(x._M_ht._M_buckets._M_impl._M_start[idx]): \n"
  while $i < $vecsize
   set $node = ($arg0._M_ht._M_buckets._M_impl._M_start[$i])
   printf "elem in bucket[%u]: \n", $i
   if $node != 0
    while $node != 0
     printf " elem[%u].left: ", $elemidx
     set $value = $node._M_val.first
     p $value
     printf " elem[%u].right: ", $elemidx 
     set $value = $node._M_val.second 
     p $value
...
define pmap
 if $argc == 0
  ...
  if $argc == 3
   printf "================map data begin =====================\n"
   while $i < $tree_size
    set $value = (void *)($node + 1)
    printf "elem[%u].left, addr:0x%x, value: ", $i, $value
    p *($arg1*)$value
    set $value = $value + sizeof($arg1)
    printf "elem[%u].right, addr:0x%x, value: ", $i, $value
    p *($arg2*)$value
    if $node._M_right != 0
...
document pdequeue_new
  pdequeue_new variable [firstidx [lastidx]]
